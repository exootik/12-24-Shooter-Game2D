...
bool GCGraphics::Initialize(Window* pWindow, int renderWidth,int renderHeight)
{
    ...
    m_pFontGeometryLoader->GenerateFontMetadata("../../res/Render/Fonts/LetterUV.txt"); 
...
GC_RESOURCE_CREATION_RESULT<GCShader*> GCGraphics::CreateShaderColor()
{
    ...
    GC_GRAPHICS_ERROR errorState = pShader->Initialize(m_pRender, "../../res/Render/Shaders/color.hlsl", "../../res/Render/CsoCompiled/color", vertexFlags, D3D12_CULL_MODE_BACK, rootParametersFlag);
    if (errorState != 0)
...
GC_RESOURCE_CREATION_RESULT<GCShader*> GCGraphics::CreateShaderTexture()
{
    ...
    GC_GRAPHICS_ERROR errorState = pShader->Initialize(m_pRender, "../../res/Render/Shaders/texture.hlsl", "../../res/Render/CsoCompiled/texture", vertexFlags, D3D12_CULL_MODE_BACK, rootParametersFlag);
    GCGraphicsLogger& profiler = GCGraphicsLogger::GetInstance();
    if (errorState != 0)
        return GC_RESOURCE_CREATION_RESULT<GCShader*>(false, nullptr, errorState);
    errorState = pShader->Load();
